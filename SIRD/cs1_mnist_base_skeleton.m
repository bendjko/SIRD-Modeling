...
for iter = 1:max_iter
    ...
    for image = 1:length(train)
    %grab the distance of each vector (excluding the last element)from its 
    %respective centroid using vec_distance function.
        [index, vec_distance] = assign_vector_to_centroid(train(image, 1:784) ,centroids(:, 1:784));
    %assign each image vector to its centroid by putting its centroid index
    %in the 785th column
        train(image, 785) = index;
        %sum the cost for each iteration and assign it to its spot in the
    %cost_iteration vector
        cost_iteration(iter, 1) = cost_iteration(iter, 1) + vec_distance;
    end
    %use update_centroids to get new centroids
...
%create a plot of the cost_iteration vector, label axes, and make title.
figure;
plot(cost_iteration);
xlabel("Iteration");
ylabel("Cost");
...
choice_initial = [5, 33, 12, 24, 31, 27, 7, 116, 147, 50, 68, 93, 19, 131, 20, 170];
centroids=data(choice_initial,:);
y=centroids;
end
...
%create a K by 785 matrix filled with zeros
new_centroids = zeros(K, 785);

%iterate for each centroid
for cents = 1:K
    %grab each image vector mapped to the centroid
    images_by_cent = data(:, 785) == cents;
    %use indexing to access each of those vectors
    vector_by_cent = data(images_by_cent, :);
    %average those vectors to create the new centroid
    new_centroids(cents, :) = mean(vector_by_cent);
end
end